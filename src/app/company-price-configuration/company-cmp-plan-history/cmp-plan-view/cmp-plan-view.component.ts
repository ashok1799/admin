...
@Component({
  selector: 'app-cmp-plan-view',
  templateUrl: './cmp-plan-view.component.html',
  styleUrls: ['./cmp-plan-view.component.css']
})
export class CmpPlanViewComponent implements OnInit {
    private login: Login;
  ...
  ngOnInit() {
        ...
        .map(res => res.json())
            .subscribe(
                    data => {this.data = data},
                    error => alert(error),
                    () => console.log('company plan view', this.data)
                );
  }
