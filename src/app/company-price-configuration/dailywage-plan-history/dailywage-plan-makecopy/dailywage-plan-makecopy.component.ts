...
@Component({
  selector: 'app-dailywage-plan-makecopy',
  templateUrl: './dailywage-plan-makecopy.component.html',
  styleUrls: ['./dailywage-plan-makecopy.component.css']
})
export class DailywagePlanMakecopyComponent implements OnInit {
private login: Login;
  ...
  ngOnInit() {
        ...
        .map(res => res.json())
            .subscribe(
                    ...
                    error => alert(error),
                    () => console.log('free plan makecopy', this.companyplan)
                );
  }
...
criteria_change(value:any, price: any, event: any){
    this.companyplan.plans[1].criteria = price;
    console.log(this.companyplan.plans[1].criteria);
}


wages_plan_make_copy(){
